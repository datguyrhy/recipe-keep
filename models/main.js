...
module.exports = (dbPoolInstance) => {
  ...
  let recipeShow = (value, callback) => {
    ...
    const queryString = `SELECT recipes.id, recipes.title,recipes.instructions,ingredients.ingredient_name FROM recipes INNER JOIN ingredtorecipe ON (recipes.id = ingredtorecipe.recipe_id) INNER JOIN ingredients ON (ingredients.id = ingredtorecipe.ingredient_id) WHERE recipes.id = $1`;
